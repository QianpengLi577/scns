...
for row in range(2):
    for col in range(2):
        ...
        img.actor.position = [(row - 0.5) * 28, 25, (col - 0.5) * 28]
...
for row in range(2):
    for col in range(2):
        ...
        img.actor.position = [(row - 0.5) * 14, 80/2, (col - 0.5) * 14]
        img.actor.orientation = [0, 90, 90]
        img.actor.force_opaque = True
...
fc1_z = np.ones_like(fc1_x) * 75 + np.random.rand(*fc1_x.shape) * 10

out_x = np.arange(10)
out_x = (out_x.ravel() - 5) * 3
out_y = np.zeros_like(out_x)
out_z = np.ones_like(out_y) + 200/2

sum_x = np.arange(10)
sum_x = (sum_x.ravel() - 5) * 6
sum_y = np.zeros_like(sum_x)
sum_z = np.ones_like(sum_y) + 125
...
acts1 = mlab.points3d(x[:x.shape[0]-10], z[:x.shape[0]-10], y[:x.shape[0]-10], s[:x.shape[0]-10], mode='cube', scale_factor=1, scale_mode='none', colormap='gray')
acts2 = mlab.points3d(x[x.shape[0]-10:], z[x.shape[0]-10:], y[x.shape[0]-10:], s[x.shape[0]-10:], mode='cube', scale_factor=2, scale_mode='none', colormap='blue-red')

# Connections between the layers
fc1 = model.fc1.weight.detach().numpy().T
out = model.fc2.weight.detach().numpy().T
fr_conv2, to_fc1 = (np.abs(fc1) > 0.2).nonzero()
...
connections = mlab.pipeline.surface(lines, colormap='gray', line_width=2.0, opacity=0.3)

# Text
mlab.text3d(z=-15*2-1.5, y=130, x=0, text='0' , scale = 3)
mlab.text3d(z=-12*2-1.5, y=130, x=0, text='1' ,scale = 3)
mlab.text3d(z=-9*2-1.5,  y=130, x=0, text='2',scale = 3)
mlab.text3d(z=-6*2-1.5,  y=130, x=0, text='3',scale = 3)
mlab.text3d(z=-3*2-1.5,  y=130, x=0, text='4',scale = 3)
mlab.text3d(z=0*2-1.5,   y=130, x=0, text='5',scale = 3)
mlab.text3d(z=3*2-1.5,   y=130, x=0, text='6',scale = 3)
mlab.text3d(z=6*2-1.5,   y=130, x=0, text='7',scale = 3)
mlab.text3d(z=9*2-1.5,   y=130, x=0, text='8',scale = 3)
mlab.text3d(z=12*2-1.5,  y=130, x=0, text='9',scale = 3)
...
# def anim():
for frame in tqdm(list(range(20*10*17*1))):
    if frame % 1 == 0:
        i = frame // 1
        ...
        acts1.mlab_source.scalars = s[:x.shape[0]-10]
        acts2.mlab_source.scalars = s[x.shape[0]-10:]
    ...
    mlab.view(azimuth=10, elevation=75, distance=250, focalpoint=[0, 90, 0], reset_roll=False)
    mlab.savefig('pic3\\frame'+str(frame)+'.png',figure=fig)    
        # yield
